...
系统现仅保留 `general_agent` 与 `xhs_trip_planner` 两条顶层路径；小红书规划链路已完成本机 Chrome
...
| 阶段 | 结果 | 提交 |
| --- | --- | --- |
| 0：基线与契约测试 | 增加登录会话与点赞选择契约 | `86a4595` |
| 1：二分类路由 | 收敛为 General Agent / XHS Planner | `e7ae850` |
| 2：MCP 登录闭环 | 完成浏览器登录、轮询、取消与 heartbeat | `5ba5d0a` |
...
| 敏感日志扫描 | 未发现日志或 `print` 输出 Token、`xsec_token` 或完整 `login_id` |

11 个跳过项均由真实高德、FlyAI、PostgreSQL 或完整 Tool Calling E2E 的显式环境开关控制。
本次默认验证没有访问真实外部供应商、触发真实网页登录或消耗真实模型额度。后端测试存在一条上游
...
- 登录会话 ID、MCP Token、Cookie、`xsec_token` 与帖子完整正文不进入普通日志或数据库。
